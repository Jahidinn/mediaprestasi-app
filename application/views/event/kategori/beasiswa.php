...
<div class="m-0 search-event">
	<div class="container bg-white mt-3 pb-5">
		...
		<div class="owl-carousel search-menu col-md-6 m-0 p-0">
			...
			<a href="#" class="active9"><strong>Beasiswa</strong></a>	
		...
		<div class="box-event">
			<div class="row m-0">
				<?php foreach ($allBeasiswa as $allBeasiswa ) : ?>
				<div class="col-md-3 p-2">
					<div class="card">
						<img src="<?= base_url(); ?>assets/img/poster-upload/<?= $allBeasiswa['image']; ?>" class="card-img-top" alt="...">
						<div class="card-body">
							<div class="judul">
								<?php if (strlen($allBeasiswa['judul']) >= 35) { $next = " ..."; } else { $next = "" ; } ?>
								<h5 class="card-title mb-1"><?= substr($allBeasiswa['judul'],0,35); ?><?= $next; ?></h5>
							</div>
							<p class="card-text m-0 mb-1"><small class="text-muted m-0"><?= $allBeasiswa['instansi']; ?></small></p>
							<hr class="m-0 p-0">
							...
							<div class="isi mt-2 mb-2">
								 <?php if (strlen($allBeasiswa['deskripsi']) >= 80) { $next2 = " ..."; } else { $next2 = "" ; } ?>
							<p class="card-text m-0 p-0"><?= substr($allBeasiswa['deskripsi'],0,80).$next2; ?></p>
							</div>
							<a href="<?= base_url(); ?>view/<?= $allBeasiswa['link']; ?>" class="btn btn-primary tombol-event"><i class="fa fa-angle-double-right"></i> Lihat</a>
						</div>
					</div>
				</div>
				<?php endforeach; ?>
